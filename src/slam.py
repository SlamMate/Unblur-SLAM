...
class SLAM:
    def __init__(self, cfg, stream:BaseDataset):
        ...
        self.online_ba = Backend(self.droid_net,self.video, self.cfg)
        ...
        self.stream = stream
        # 在 self.stream = stream 之后添加：
        self.timing_stats = {
            'online_inference_time': 0.0,
    ...
    def terminate(self):
        ...
        if self.cfg['tracking']['backend']['final_ba'] and self.cfg['mapping']['eval_before_final_ba']:
            ...
            try:
                if not self.cfg['dataset'] == 'exblurf_motion':
            ...
            if not self.only_tracking: 
                ...
                if self.cfg['dataset'] == 'exblurf_motion':
                    global_scale = 1.0
        ...
        if not self.cfg['dataset'] == 'exblurf_motion':
            # 保存运行时统计信息
            import json
            stats_path = f'{self.save_dir}/runtime_stats.json'
            ...
            self.printer.print(f'Online inference time: {self.timing_stats["online_inference_time"]:.2f}s', FontColor.EVAL)
            self.printer.print(f'Total inference time: {self.timing_stats["total_inference_time"]:.2f}s', FontColor.EVAL)
            self.printer.print(f'Peak GPU memory: {self.timing_stats["peak_gpu_memory"]:.2f}GB', FontColor.EVAL)
            self.printer.print(f'Peak CPU memory: {self.timing_stats["peak_cpu_memory"]:.2f}GB', FontColor.EVAL)
    ...
    def run(self):
        self.start_time_total = time.time()  # 添加这行
        self.start_time_online = time.time()  # 添加这行
...
def gen_pose_matrix(R, T):
    pose = np.eye(4)
    pose[0:3, 0:3] = R.detach().cpu().numpy()  # 添加 .detach()
    pose[0:3, 3] = T.detach().cpu().numpy()    # 添加 .detach()
    return pose