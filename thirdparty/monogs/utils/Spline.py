...
def exp_r2q_parallel(r, eps=1e-4):
    x, y, z = r[..., 0], r[..., 1], r[..., 2]
    theta = 0.5 * torch.sqrt(torch.clamp(x**2 + y**2 + z**2, min=1e-10))
...
def log_q2r_parallel(q, eps_theta=1e-5, eps_w=1e-5):
    x, y, z, w = q[..., 0], q[..., 1], q[..., 2], q[..., 3]

    theta = torch.sqrt(torch.clamp(x**2 + y**2 + z**2, min=1e-10))
    ...
    lambda_ = torch.where(
        ...
        torch.where(
            bool_w_0_left,
            log_q2r_lim_w_0_left(theta + 1e-6),
            log_q2r_lim_w_0_right(theta + 1e-6),
        ),
        torch.where(
            bool_theta_0,
            log_q2r_taylor_theta_0(w, theta),
            log_q2r(w, theta + 1e-6)
        ),
    )
...
def SplineN_cubic(pose0, pose1, pose2, pose3, poses_number, NUM):
    ...
    r_01 = quaternion_to_axis_angle(q_01.squeeze(-1)) * coeff1_r  # [4]
    r_12 = quaternion_to_axis_angle(q_12.squeeze(-1)) * coeff2_r  # [5]
    r_23 = quaternion_to_axis_angle(q_23.squeeze(-1)) * coeff3_r  # [6]

    print(r_01.shape, r_12.shape, r_23.shape)

    q_t_0 = axis_angle_to_quaternion(r_01)  # [7]
    q_t_1 = axis_angle_to_quaternion(r_12)  # [8]
    q_t_2 = axis_angle_to_quaternion(r_23)  # [9]
