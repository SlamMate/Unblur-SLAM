...
from thirdparty.BAGS.scene.kernelnet_single_res import BlurKernel as BlurKernel_ss
from thirdparty.BAGS.scene.kernelnet_multires import BlurKernel as BlurKernel_ms
...
class GaussianModel:
    def __init__(self, sh_degree: int, config=None):
        ...
        if self.config["deblur"]["open"]:
            ...
            inp_shape = [max_keyframe, self.config['cam']['H_out'], self.config['cam']['W_out']]
            
            # Multi-scale network (for stages 1-3)
            self.mlp_rgb_ms = BlurKernel_ms(
                inp_shape[0], inp_shape[1], inp_shape[2],
                ks1=self.config["deblur"]["ks1"],
                ks2=self.config["deblur"]["ks2"], 
                ks3=self.config["deblur"]["ks3"],
                not_use_rgbd=self.config["deblur"]["not_use_rgbd"],
                not_use_pe=self.config["deblur"]["not_use_pe"]
            ).to("cuda")
            
            # Single-scale network (for final refinement)
            self.mlp_rgb_ss = BlurKernel_ss(
                inp_shape[0], inp_shape[1], inp_shape[2],
                ks=self.config["deblur"]["ks_ss"],
                not_use_rgbd=self.config["deblur"]["not_use_rgbd"],
                not_use_pe=self.config["deblur"]["not_use_pe"]
            ).to("cuda")

            self.mlp_base_lr = None  # 保存MLP的基础学习率
    ...
    def set_mlp_learning_rate(self, lr_multiplier: float):
        """
        只调整MLP的学习率，不影响Gaussian参数
        lr_multiplier: 学习率倍数
        """
        ...
        for group in self.optimizer.param_groups:
            # 只修改MLP相关的参数组
            if group.get("name", None) is None:  # MLP参数没有name字段
                if self.mlp_base_lr is not None:
                    group['lr'] = self.mlp_base_lr * lr_multiplier
    ...
    def create_pcd_from_image(self, cam_info, init=False, scale=2.0, depthmap=None):
        ...
        if depthmap is not None:
            ...
            if self.config["mapping"]["sensor_type"] == "monocular":
                depth_raw = (
    ...
    def create_pcd_from_image_and_depth(self, cam, rgb, depth, init=False):
        if init:
            ...
            downsample_factor = self.config["mapping"]["pcd_downsample"]
        point_size = self.config["mapping"]["point_size"]
        ...
        W2C = getWorld2View2(cam.R.detach(), cam.T.detach()).cpu().numpy()
        # W2C = getWorld2View2(cam.R, cam.T).cpu().numpy()
        if cam.deblur_fail:
            R, t, theta, rho = cam.get_mid_extrinsic()
            W2C = getWorld2View2(R, t).detach().cpu().numpy()
        ...
        # The scale of the Gaussians are initialized related to their distance to the other
        # Gaussians added in the same batch.
        scales = torch.log(torch.sqrt(dist2))[..., None]
        if not self.isotropic:
            # 3 dimensions initialized isotropically if we optimize anisotropic Gaussians
            scales = scales.repeat(1, 3)

        rots = torch.zeros((fused_point_cloud.shape[0], 4), device="cuda")
        # Set rotation to identity for initialization
        rots[:, 0] = 1
        # Set opacity to 0.5
    ...
    def training_setup(self, training_args):
        ...
        # In training_setup of GaussianModel
        if self.config["deblur"]["open"]:
            # 从配置或环境变量获取MLP学习率
            mlp_lr = float(os.environ.get('KERLR', 5e-10))
            self.mlp_base_lr = mlp_lr
            if self.config and "deblur" in self.config:
                mlp_lr = float(self.config["deblur"].get("mlp_lr", mlp_lr))
            
            print(f'MLP learning rate: {mlp_lr}')
            
            for p in self.mlp_rgb_ss.parameters():
                l += [{'params': [p], 'lr': mlp_lr}]
    ...
    def densify_and_split(self, grads, grad_threshold, scene_extent, N=2):
        ...
        # Concatenate the new features to the global Gaussian parameters
        self.densification_postfix(
            new_xyz,
            new_features_dc,
            new_features_rest,
    ...
    def densify_and_prune(self, max_grad, min_opacity, extent, max_screen_size):
        ...
        if max_screen_size:
            ...
            if min_opacity == 0.005:
                big_points_ws = self.get_scaling.max(dim=1).values > 0.1 * extent
                prune_mask = torch.logical_or(torch.logical_or(prune_mask, big_points_vs), big_points_ws)
            elif min_opacity == 0.1:
