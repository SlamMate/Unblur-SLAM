...
def render_virtual(
    ...
    #R, t, theta, rho = viewpoint_camera.get_virtual_extrinsic(virtual_cam_idx)
    # Create zero tensor. We will use it to make pytorch return gradients of the 2D (screen-space) means
    if pc.get_xyz.shape[0] == 0:
        return None
    ...
    raster_settings = GaussianRasterizationSettings(
        ...
        bg=bg_color,
        scale_modifier=scaling_modifier,
        viewmatrix=viewpoint_camera.world_view_transform_custom(R,t),
        projmatrix=viewpoint_camera.full_proj_transform_custom(R,t),
        projmatrix_raw=viewpoint_camera.projection_matrix,
        sh_degree=pc.active_sh_degree,
    ...
    if colors_precomp is None:
        if pipe.convert_SHs_python:
            ...
            dir_pp = pc.get_xyz - viewpoint_camera.camera_center_custom(R,t).repeat(
                pc.get_features.shape[0], 1
            )
            dir_pp_normalized = dir_pp / dir_pp.norm(dim=1, keepdim=True)
            sh2rgb = eval_sh(pc.active_sh_degree, shs_view, dir_pp_normalized)
