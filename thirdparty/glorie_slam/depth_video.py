...
class DepthVideo:
    ...
    def __item_setter(self, index, item):
        ...
        if item[4] is not None:
            ...
            if torch.isnan(self.mono_disps[index]).any():
                print("NaN detected in mono_disps in item setter")
                #set them to 1e-5
                self.mono_disps[index][torch.isnan(self.mono_disps[index])] = 1e-5
    ...
    def dspo(self, target, weight, eta, ii, jj, t0=1, t1=None, itrs=2, lm=1e-4, ep=0.1, motion_only=False, opt_type="pose_depth"):
        ...
        with self.get_lock():
            ...
            if opt_type == "pose_depth":
                ...
                self.validate_tensor(target, 'target')
                self.validate_tensor(weight, 'weight')
                self.validate_tensor(eta, 'eta')
                self.validate_tensor(ii, 'ii')
                self.validate_tensor(jj, 'jj')
                ...
                # **Check for NaNs in self.disps**
                if torch.isnan(self.disps).any():
                    print("NaNs detected in self.disps after BA optimization")
                    # Handle NaNs appropriately
                    self.disps = torch.where(torch.isnan(self.disps), torch.zeros_like(self.disps), self.disps) 
                self.disps.clamp_(min=1e-5)

                #check nan in the other variables
                if torch.isnan(self.poses).any():
                    print("NaN detected in poses in DSPO pose_depth")
                    return False
                
                if torch.isnan(self.disps).any():
                    print("NaN detected in disps in DSPO pose_depth")
                    #set them to 1e-5
                    self.disps[torch.isnan(self.disps)] = 1e-5
                ...
                if torch.isnan(self.disps).any():
                    print("NaNs detected in self.disps after depth_scale optimization")
                    # Handle NaNs appropriately
                    self.disps = torch.where(torch.isnan(self.disps), torch.zeros_like(self.disps), self.disps)

                if torch.isnan(self.poses).any():
                    print("NaNs detected in self.poses after depth_scale optimization")
                    # Handle NaNs appropriately
                    self.poses = torch.where(torch.isnan(self.poses), torch.zeros_like(self.poses), self.poses)

                self.disps.clamp_(min=1e-5)

                if torch.isnan(self.disps).any():
                    print("NaN detected in disps in DSPO depth_scale")
    ...
    def validate_tensor(self, tensor, name):
        if torch.isnan(tensor).any():
            print(f"NaNs detected in {name}")
        if torch.isinf(tensor).any():
            print(f"Infs detected in {name}")
        if torch.isfinite(tensor).all() == False:
            print(f"Invalid values detected in {name}")
    ...
    def get_depth_and_pose(self,index,device):
        with self.get_lock():
            ...
            #print(f"est_disp: {est_disp.sum()}, est_depth: {est_depth.sum()}, depth_mask: {depth_mask.sum()}")
    ...
    def update_valid_depth_mask(self,up=True):
        ...
        if torch.isnan(disps).any():
            print("NaN detected in disps in update_valid_depth_mask")
            #set them to 1e-5
            disps[torch.isnan(disps)] = 1e-5 
        ...
        if torch.isnan(depths_median).any():
            print("Warning: NaNs detected in depths_median. Replacing NaNs with zeros.")
            depths_median = torch.where(torch.isnan(depths_median), torch.zeros_like(depths_median), depths_median)
        ...
        if torch.isnan(self.disps_up).any():
            print("NaN detected in disps in DSPO pose_depth")
            #set them to 1e-5
            self.disps_up[torch.isnan(self.disps_up)] = 1e-5 
